...
2. 先导入老师提供的班级照片（二选一）：
   ...
   也可以登录教师账号后，在“学生/人脸库 -> 批量导入人脸图片”直接多选 `jpg/png` 文件。文件名格式为 `学号-姓名-专业-性别.jpg/png`，例如 `2022302181209-吴悦-网络空间安全-女.jpg`；系统会自动创建/更新学生并加入人脸样本。

3. 在“学生/人脸库”中确认学生和人脸样本数，也可新增学生、批量导入图片、点击“样本”预览/删除单张样本，或用摄像头补采样本。
...
| 活体检测 | `analyze_liveness()`：10 种单动作池 + 3 组限时挑战 + 多帧人脸质量/纹理/运动检测 + 实时随机打光 |
| 人脸库 | 学生 CRUD + 查看/预览/删除单个人脸样本 + 单学生多图上传 + 按 `学号-姓名-专业-性别.jpg/png` 批量导入人脸图片 + `face_data` 脚本导入 + 摄像头补采样本；同一学生多样本会按学生聚合得分 |
...
- 如果老师追问“如何抵御预录视频”，点击 **展示随机挑战抗视频**，可直接展示 10 种单动作池、3 组随机挑战、每组 5 秒限时、90 秒过期、实时随机打光和多帧运动检查逻辑。
